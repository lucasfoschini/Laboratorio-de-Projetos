  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      <Link href="/projetos" className="group bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-2xl p-5 shadow-card hover:shadow-card-md hover:border-neutral-300 dark:hover:border-neutral-600 transition-all">
        ...
        <p className="font-display font-bold text-sm text-neutral-900 dark:text-neutral-100 group-hover:text-brand-700 dark:group-hover:text-brand-400 transition-colors">Projetos</p>
        <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-0.5 leading-snug">Explore e participe de projetos ativos</p>
      </Link>
      ...
      {isAuthenticated && (
        <Link href="/projetos/novo" className="group bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-2xl p-5 shadow-card hover:shadow-card-md hover:border-brand-200 dark:hover:border-brand-700 transition-all">
          <div className="w-10 h-10 rounded-xl bg-brand-50 dark:bg-brand-950 flex items-center justify-center mb-3">
            <FlaskConical size={22} className="text-brand-600 dark:text-brand-400" />
          </div>
          <p className="font-display font-bold text-sm text-neutral-900 dark:text-neutral-100 group-hover:text-brand-700 dark:group-hover:text-brand-400 transition-colors">Criar projeto</p>
          <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-0.5 leading-snug">Inicie um novo projeto de extensão</p>
        </Link>
      )}
    </div>
  );
